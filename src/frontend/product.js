...
const productApp = new Vue({
    ...
    methods: {
        ...
        fetchPhotos: function(){
            // axios.get(`/api/product/photos/${this.sharedState.extra.product.id}`).then(resp =>{
            //     this.photos = resp.data;
            // }).catch(err=>{});

            axios.get(`/api/product/photos/${this.sharedState.extra.product.id}/p_sp`).then(resp =>{
                this.pspPhotos = resp.data;
            }).catch(err=>{});
    ...
    filters : {
        customURL(value){
            if(value.indexOf('p_sp/')) {
                return value.replace('p_sp/', '');
            }else {
                return value;
            }
